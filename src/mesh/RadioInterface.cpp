...
RDEF(UNSET, RX_FREQ0-0.020, RX_FREQ0+0.020, 100, 0, 22, true, false, false),
    RDEF(US, RX_FREQ1-0.020, RX_FREQ1+0.020, 100, 0, 22, true, false, false),
    RDEF(EU_433, RX_FREQ2-0.020, RX_FREQ2+0.020, 100, 0, 22, true, false, false),
    RDEF(EU_868, RX_FREQ3-0.020, RX_FREQ3+0.020, 100, 0, 22, false, false, false),
    RDEF(CN, RX_FREQ4-0.020, RX_FREQ4+0.020, 100, 0, 22, true, false, false),
    RDEF(JP, RX_FREQ5-0.020, RX_FREQ5+0.020, 100, 0, 22, true, false, false),
    RDEF(ANZ, RX_FREQ6-0.020, RX_FREQ6+0.020, 100, 0, 22, true, false, false),
    RDEF(KR, RX_FREQ7-0.020, RX_FREQ7+0.020, 100, 0, 22, true, false, false),
    RDEF(TW, RX_FREQ8-0.020, RX_FREQ8+0.020, 100, 0, 22, true, false, false),
    RDEF(RU, RX_FREQ9-0.020, RX_FREQ9+0.020, 100, 0, 22, true, false, false),
    RDEF(IN, RX_FREQ10-0.020, RX_FREQ10+0.020, 100, 0, 22, true, false, false),
    RDEF(NZ_865, ST_FREQ11, END_FREQ11, 100, 0, 22, true, false, false),
    RDEF(TH, ST_FREQ12, END_FREQ12, 100, 0, 22, true, false, false),
    RDEF(LORA_24, ST_FREQ13, END_FREQ13, 100, 0, 22, true, false, false)
 };
...
void initRegion()
{
    ...
    for (; r->code != config.lora.region; r++)
        ;
    myRegion = r;
...
void RadioInterface::applyModemConfig()
{
    ...
    if (loraConfig.use_preset) {
        ...
        case Config_LoRaConfig_ModemPreset_MEDIUM_FAST:
            bw = (myRegion->wideLora) ? 31.25 : 31.25;
            cr = 5;
            sf = 6;
            break;
        ...
        case Config_LoRaConfig_ModemPreset_LONG_SLOW:
            bw = (myRegion->wideLora) ? 20.83 : 20.83;
            cr = 6;
            sf = 7;
            break;
        ...
        if (bw == 20) // This parameter is not an integer
            bw = 20.83;
    ...
    if (power == 0)
        power = 22; // Default to default power if we don't have a valid power
    ...
    float forFREQ = myRegion->freqStart + 0.020;
    double formulaFREQ = std::round(forFREQ*1000)/1000.0;
    float freq;
    DEBUG_MSG("formulaFREQ: %f\n", formulaFREQ);
   
    if (formulaFREQ == RX_FREQ0) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ1) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ2) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ3) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ4) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ5) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ6) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    } else if (formulaFREQ == RX_FREQ7) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
       } else if (formulaFREQ == RX_FREQ8) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
       } else if (formulaFREQ == RX_FREQ9) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
       } else if (formulaFREQ == RX_FREQ10) {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
        } else if (formulaFREQ == ST_FREQ11) {
     freq = myRegion->freqStart + (bw / 2000) + ( channel_num * (bw / 1000));
          } else if (formulaFREQ == ST_FREQ12) {
     freq = myRegion->freqStart + (bw / 2000) + ( channel_num * (bw / 1000));
          } else if (formulaFREQ == ST_FREQ13) {
     freq = myRegion->freqStart + (bw / 2000) + ( channel_num * (bw / 1000));
    } else  {
     freq = ((myRegion->freqEnd + myRegion->freqStart) / 2);
    }
