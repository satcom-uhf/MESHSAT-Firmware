...
{
     DEBUG_MSG("configHardwareForSend\n");
     float ftfreq;
    float ffrfreq = RadioLibInterface::instance->getFreq();
    double frfreq = std::round((ffrfreq - config.lora.frequency_offset) *1000)/1000;
    
    DEBUG_MSG("Setting RX freq:%f\n",frfreq);
    if (frfreq == RX_FREQ0) {
     ftfreq = TX_FREQ0;
    } else if (frfreq == RX_FREQ1) {
     ftfreq = TX_FREQ1;
    } else if (frfreq == RX_FREQ2) {
     ftfreq = TX_FREQ2;
    } else if (frfreq == RX_FREQ3) {
     ftfreq = TX_FREQ3;
    } else if (frfreq == RX_FREQ4) {
     ftfreq = TX_FREQ4;
    } else if (frfreq == RX_FREQ5) {
     ftfreq = TX_FREQ5;
    } else if (frfreq == RX_FREQ6) {
     ftfreq = TX_FREQ6;
    } else if (frfreq == RX_FREQ7) {
     ftfreq = TX_FREQ7;
    } else if (frfreq == RX_FREQ8) {
     ftfreq = TX_FREQ8;
     } else if (frfreq == RX_FREQ9) {
     ftfreq = TX_FREQ9;
     } else if (frfreq == RX_FREQ10) {
     ftfreq = TX_FREQ10;
    } else  {
     ftfreq = ffrfreq;
    }

    DEBUG_MSG("Setting TX freq:%f\n",ftfreq);
    lora.setFrequency(ftfreq + config.lora.frequency_offset);
...
template<typename T>
void SX126xInterface<T>::configHardwareBack()
{

    setStandby();
    DEBUG_MSG("Setting RX freq:%f\n",RadioLibInterface::instance->getFreq());
    lora.setFrequency(RadioLibInterface::instance->getFreq());
    startReceive();   

    RadioLibInterface::configHardwareBack();
}
